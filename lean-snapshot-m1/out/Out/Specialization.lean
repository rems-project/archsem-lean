...
/- MODIFICATION: dont pass RegisterRef `Register` and `RegisterType` args. This is done throgh [Arch] arg now. -/
...
/- MODIFICATION: remove ConcurrencyInterfaceV1 namespace and its contents. -/
/-
...
/- MODIFICATION: open ArchSem here to access `Arch` type. -/
open ArchSem

/- MODIFICATION: remove [Arch] arguments to these sail_* functions. -/
...
/- sail_mem_write returns `Result Unit abort` instead of `Result (option bool) abort` -/
def sail_mem_write (req : Mem_request n nt Arch.addr_size Arch.addr_space Arch.mem_acc) (valueBytes : Vector (BitVec 8) n) (tags : Vector Bool nt) :
    SailM (Result Unit Arch.abort) := do
