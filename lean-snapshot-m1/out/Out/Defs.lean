...
/- MODIFICATION : moved this instance definition up so it can be used below. -/
/- MODIFICATION : Add `ArchSem.` prefix. -/
@[reducible]
instance : ArchSem.Arch where
  /- MODIFICATION : Added register,register_target fields. -/
  register := Register
  ...
  tlbi := Unit
  sys_reg_id := Unit
  /- MODIFICATION : I commented out the fields I'm not using. -/
  /-
  CHERI := false
  cap_size_log := 0
...
/- MODIFICATION : RegisterRef no longer takes RegisterType as an argument. -/
instance : Inhabited (RegisterRef (BitVec 64)) where
  default := .Reg _PC
abbrev exception := Unit

/- MODIFICATION: PreSailM no longer takes RegisterType or choice source arguments. -/
