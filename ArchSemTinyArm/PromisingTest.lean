...
namespace MPDMBS
/-
The classic MP litmus test with memory barriers.
...
def initialMem : MemoryMap :=
  ...
  /- Thread 0 @ 0x500 -/
  |>.write 4 0x500 0xf8206822  /- STR X2, [X1, X0] -/
  |>.write 4 0x504 0xd5033fbf  /- DMB SY -/
  |>.write 4 0x508 0xf8236885  /- STR X5, [X4, X3] -/
  /- Thread 1 @ 0x600 -/
  |>.write 4 0x600 0xf8636885  /- LDR X5, [X4, X3] -/
  |>.write 4 0x604 0xd5033fbf  /- DMB SY -/
  |>.write 4 0x608 0xf8606822  /- LDR X2, [X1, X0] -/
  /- Backing memory so the addresses exist -/
  |>.write 8 0x1100 0x0
  |>.write 8 0x1200 0x0
...
def terminationCondition : TerminationCondition nThreads := fun tid regs =>
  match tid with
  | 0 => regs.get? ._PC == .some (BitVec.ofNat 64 0x50c)
  | 1 => regs.get? ._PC == .some (BitVec.ofNat 64 0x60c)
